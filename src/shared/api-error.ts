...
export class ApiError extends Error {
  constructor(readonly status: 400 | 401 | 404 | 409 | 503, message: string, readonly code?: string) {
    super(message);
  }
}

export function sendApiError(error: unknown, response: Response, next: NextFunction) {
  if (error instanceof ApiError) return response.status(error.status).json({ error: error.message, ...(error.code ? { code: error.code } : {}) });
