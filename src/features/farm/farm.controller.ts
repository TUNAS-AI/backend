...
export const farmController = {
  ...
  snapshot: async (request: Request, response: Response, next: NextFunction) => { try { const snapshot = await service.snapshot(owner(request)); response.json({ farm: serializeRecord(snapshot.farm), fieldBlocks: snapshot.fieldBlocks.map((field) => serializeRecord(field as Record<string, unknown>)), cropBatches: snapshot.cropBatches.map((batch) => serializeRecord(batch as Record<string, unknown>)) }); } catch (error) { sendApiError(error, response, next); } },
