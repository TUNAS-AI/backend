...
function repository(deleted: string[]) {
  return {
    ...
    async delete(ownerId: string, farmId: string) { deleted.push(`${ownerId}:${farmId}`); },
  };
}
...
test("cleans farm calendar events before deleting local farm data", async () => {
  ...
  assert.deepEqual(calls, ["calendar:farm-1", "owner-1:farm-1"]);
});
...
test("calendar cleanup failure does not block local farm deletion", async () => {
  ...
  assert.deepEqual(deleted, ["owner-1:farm-1"]);
  assert.deepEqual(warnings, [["Google Calendar cleanup failed during farm deletion", { farmId: "farm-1", kind: "Error" }]]);
});

test("deletes Telegram identity data and missions before the farm inside one transaction", () => {
  const source = readFileSync(resolve("src/features/farm/farm.repository.ts"), "utf8");
  assert.match(source, /\$transaction\(async \(tx\)[\s\S]*tx\.telegramLinkToken\.deleteMany\(\{ where: \{ userId: ownerId \} \}\)[\s\S]*tx\.telegramConnection\.deleteMany\(\{ where: \{ userId: ownerId \} \}\)[\s\S]*tx\.mission\.deleteMany\(\{ where: \{ farmId \} \}\)[\s\S]*tx\.farm\.delete\(\{ where: \{ farmId \} \}\)/);
});
