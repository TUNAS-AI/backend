...
function parseSchema<T>(schema: z.ZodType<T>, value: unknown, label: string): T { const result = schema.safeParse(value); if (!result.success) throw new ApiError(400, `${label} is invalid`); return result.data; }
export function parsePreviewInterpret(value: unknown) { const body = input(value); const messages = body.messages === undefined ? [] : parseSchema(z.array(messageSchema).max(39), body.messages, "messages"); return { previewId: body.previewId === undefined ? undefined : uuid(body.previewId, "previewId"), messages: messages as MessageInput[], message: text(body.message, "message"), facts: body.facts === undefined ? undefined : parseSchema(factsSchema, body.facts, "facts") }; }
...
export function parseReplanConfirmation(value: unknown) { return parseConfirmation(value); }
export function parseCalendarRange(value: unknown) { const query = input(value); const from = text(query.from, "from"); const to = text(query.to, "to"); if (!z.string().date().safeParse(from).success || !z.string().date().safeParse(to).success || from > to) throw new ApiError(400, "Calendar range is invalid"); return { from: new Date(`${from}T00:00:00.000Z`), to: new Date(`${to}T00:00:00.000Z`) }; }
...
export function parseCloseout(value: unknown): MissionCloseoutInput { const body = input(value); const optionalNumber = (field: string) => body[field] === undefined || body[field] === null ? null : number(body[field], field, 0); if (typeof body.dryingCompleted !== "boolean") throw new ApiError(400, "Closeout decisions are invalid"); return { actualHarvestKg: number(body.actualHarvestKg, "actualHarvestKg", 0), actualDriedKg: number(body.actualDriedKg, "actualDriedKg", 0), harvestedAreaHectares: optionalNumber("harvestedAreaHectares"), dryingCompleted: body.dryingCompleted, rejectedKg: optionalNumber("rejectedKg"), notes: body.notes === undefined || body.notes === null ? null : text(body.notes, "notes") }; }
